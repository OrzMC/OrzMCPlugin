...
- **同步预检 `VERDICT=ok` 被误判为"需人工处理"**：`main-develop-sync.yml` 的 `case` 里 `manual|*)` 含 `*` 兜底却排在 `ok)` 之前，
  ...
  修复：把 `ok)` 前移到兜底之前，并在 workflow 内注明该分支顺序陷阱。

### 📺 视频系列（新增：宣传短片 + 功能分集脚手架，零产物可重生成）

- **仓库策略**：`videos/` 只跟踪**源与工具**（`episodes/*.yml` 唯一事实源、`tools/*` 生成器、`brand` 与系列文档）；
  产物（SRT / 卡片 / TTS 样音 / 成片 / 原始录屏）一律**不入库**，随时可由 `make.sh <epNN\|all>` 重建
  （`.build/` 已 gitignore；`verify.sh` 校验零产物 + 源约束 + **幂等**，两次生成逐字节一致）
- **分集规格**：EP0 宣传短片（≤90s，聚焦三大卖点）+ EP1–EP25 功能分集（**一集一个功能点**，单集**硬上限 180s**，
  目标 120–150s）；时长 / 语速（3.4–5.2 字/秒）/ 镜头与信息点上限 / 字幕行宽与单句时长 / facts 与文档锚点 /
  隐私六类约束进 CI 门禁（`VideoScriptConsistencyTest`）
- **迭代更新机制**：`coverage.yml`（功能点↔集号）+ `affected-episodes.py`（L1 换字幕不重录 / L2 单镜重录 +
...
### ✨ 新增（新手指南书配置格式 v2：一看就懂）
- **guide_book.yml 简化格式**：一行 = 一句话、一页 = 一个 `-` 列表，样式/链接只需 3 个记号
  （`**粗体**` / `<u>下划线</u>` / `[文字](https://地址 '悬停提示')`），告别 `text:`/`link:` 双层嵌套、
  `content` 键双重含义与藏在条目里的 `page_break`；需要颜色时用 `text:`/`link:` 全写法（简写可混用）。
  3 分钟上手手册：[`docs/guide-book.md`](docs/guide-book.md)
- **旧 `content:` 格式完全兼容**：两种格式都可读；解析失败时按纯文本兜底（内容不丢）并给出**页号/行号**定位的日志告警

### ✨ 新增（guide_book 旧格式自动迁移 + 健康校验 + 文档一致性门禁）
- **旧 `content:` 格式启动自动迁移为 v2**：备份 `guide_book.yml.bak` → 生成带速查注释的新文件 → **回读校验**
  （新文件解析后必须与迁移前逐页逐行一致）→ 同目录原子写；任一步失败/不一致即**放弃迁移并保留原文件**（只记日志）。
  幂等：已是 v2 则零动作（不重写、不刷备份）
- **修复启动回写抹掉 guide_book.yml 注释**：移除 `guide_book.yml` 的空 `setDefaults` 调用（其副作用是
  `saveConfig` 回写，而 `YamlConfiguration` 不保留注释）——服主手写的注释/文档头不再被启动流程清空
- **guide_book 段进入配置健康检查**：`enable`/`title`/`author` 类型 + 解析器同口径问题（含**页号/行号**）
  统一在启动告警中列出
- **文档一致性测试（防漂移）**：`GuideBookDocsConsistencyTest` 钉死「手册 `docs/guide-book.md` 的规范示例块」
  「内置模板 `guide_book.yml`」「迁移产物头部注释」三者可解析且覆盖 3 个记号 —— 以后改格式不改文档/模板会直接挂 CI

### 🐛 修复（guide_book 看着支持其实不支持的坑）
- **颜色口径统一**：命名色（`red`/`AQUA`）与 `#RRGGBB` 均生效；非法值忽略并告警（原先命名色静默失效）
- **空值不再抛异常**：`bold:` / `color:` 写空值时不再因拆箱/解引用 NPE 导致 `/guide` 报错（样式模型改为不可变记录，解析期兜底）
- **未知条目不再产出空行**：无法识别的行不再"日志说跳过、实际仍入列"（改为跳过或纯文本兜底 + 可定位告警）
- **超上限降级而非报错**：单页 1024 字符 / 100 页超限时省略末尾内容并告警（避免 Paper 侧校验直接抛异常）
- **解析结果进程内缓存**：删除"每次 `/guide`、每个新玩家进服都读盘 + 全量 YAML 重解析"的路径；
  `/orzmc config reload`（或 `reload guide_book`）触发缓存失效，编辑即改即生效

### 🐛 修复（QQ 被动回复配额导致分页消息丢失）
- **配额用尽自动降级为主动消息**：同一入站消息的被动回复超过官方上限（群 5 / 单聊 4）后，后续消息不再被平台以 `40034128` 拒收，
  而是自动改走主动通道 —— 修复 `$w` / `$v` 列表分页、长文分段、提示+正文组合等「一次入站发多条」流程**只发出前几条**的问题
- **`40034128` 拒收兜底重投**：被动通道被平台拒（未投递）时改用主动消息重投一次（确定性失败才重试，不违反 D7，不会重复通知）
- 越限 WARN 改为**同一 `msg_id` 只打一次**（原实现每超限一条打一次，长列表会刷屏）
...
- **README(en/zh)**：build / Publish 徽章显式 pin `?branch=main`（与 codecov `branch=main` 口径一致）；参与贡献行注明「日常 PR 打 `develop`，`main` 为发布线」；发布手册补 §5.5 踩坑记录（#446）
