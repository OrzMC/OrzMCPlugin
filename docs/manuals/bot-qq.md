...
| 广播/通知失败且日志为 `投递网络异常（不重试）: ... java.net.http.HttpConnectTimeoutException` | **连接阶段**超时（3s 未能建连）：查服务器出站网络 —— QQ 白名单是否含**实际出口 IP**（走代理则是代理出口 IP）、防火墙/安全组是否放行 `api.bot.qq.com:443` |
| 同样的失败但日志为 `java.net.http.HttpTimeoutException: request timed out` | **请求阶段**超时（连上了但 8s 内无响应）：多为平台侧/中间链路抖动或限流，可观察是否与高频主动消息同时出现（控制频率） |
