...
- 单条文本上限：**QQ 官方未公开数字**（只给错误码 `40054007 消息长度超限`）——默认按 **3000 字节**保守分段（配置项 `im.yml → platforms.qq.max_text_bytes`，0 = 不分段），并用 `docs/dev/qq-text-limit-probe.sh` 在测试群实测后固化；分段最多 5 段（受被动回复次数上限约束），超出截断并告警（完整内容见服务器日志）；
