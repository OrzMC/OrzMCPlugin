...
- **被动回复（带 `msg_id`）**：官方（2026-09）有效窗口 **群聊 5 分钟 / 单聊 60 分钟**，每条源消息最多回复 **群聊 5 次 / 单聊 4 次**（超限平台返 `40034128`）；插件会持续递增 `msg_seq`（官方要求，**相同 `msg_id` + `msg_seq` 重复发送会失败** `40054005`）；
- **一次入站发多条时的配额自动降级（2026-09-13 修复）**：列表分页（`$w` / `$v`）、长文分段、提示+正文组合等都会对**同一条源消息**连发多条被动回复，超过上限（群 5 / 单聊 4）时平台会以 `40034128` 拒收**后续全部内容**。插件的处理：
  1. **预判降级**：同一 `msg_id` 的被动回复用满配额后，后续消息**自动改走主动消息**（不带 `msg_id`/`msg_seq`）→ 分页/分段内容全部送达，不再丢页；该 `msg_id` 首次越限时打**一条** WARN（含上限值与 `40034128`，不刷屏）；
  2. **拒收兜底**：若平台仍以 `400 + 40034128` 拒收（例如 5 分钟窗口已过期），说明**平台未投递**，插件用主动消息**重投一次**（确定性失败才重试，不违反 D7「不重试」，也不会重复通知）；
  - 注意：降级后消耗的是**主动消息频控**（群 60 或 30/qpm、单关系 20/qpm、每群 1000 条/天）。超长列表（数十页）仍可能触及主动频控——此时请用 `$w <页码>` / `$v <页码>` 查看单页；
- **主动消息（不带 `msg_id`）频控**（官方 2026-09）：群聊 Bot 维度 **60/qpm**（企业/个人实名认证）或 30/qpm（未认证），单关系维度 20/qpm，每群 **1000 条/天**；单聊 1000 条/用户/天。与插件现有聚合/节流配合足够；
- 单条文本上限：**2026-09 实机实测 ≥150KB**（ASCII 96KB / 中文 150KB 均 200 且投递成功；官方仍未公开数字，仅错误码 `40054007`）→ 通知场景等同无上限。默认 `im.yml → platforms.qq.max_text_bytes: 32768` 仅作**防御性上限**（超长模板最多分 5 段，超出截断 + 告警；0 = 不分段）；复测见 `docs/dev/qq-text-limit-probe.sh`；
- **openid 按 app 隔离**：同一群在不同机器人（app_id）下 openid 不同——更换/重建机器人后必须重新获取并绑定，否则投递报 `11255 请求的资源不存在（用户/群已注销）`；
- **长消息分段**（D2）：超过单条上限时拆成多段依次发送（优先在换行/句末切，不切碎字符）；被动回复每段递增 `msg_seq`（官方要求）；TG 4096 字符 / Discord 2000 字符 / 飞书 150KB（实际不分段）；
- 消息内容含 URL 会被平台拒（错误码 `40054010`）——通知模板请避免链接，或先在开放平台完成 URL 相关配置；
