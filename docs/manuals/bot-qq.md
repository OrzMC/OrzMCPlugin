...
| 日志 `投递响应超时（结果未知：平台可能已投递，未重试）: ... HttpTimeoutException: request timed out` | **请求阶段**超时（连上了但 30s 内无响应）：**多数情况平台已投递，只是响应迟到/丢失**（2026-09 线上实测：主动消息响应可 >10s 而群里实际已收到），故不重试（避免重复通知），也**不计入健康 lastError**。若频发，说明主动消息频率偏高（D14 配额/频控），降低广播频率；请按群内是否实际收到为准判定 |
...
- 投递结果三态：`SENT`（2xx）/ `FAILED`（确定失败，污染健康 lastError）/ `UNKNOWN`（响应超时，平台可能已投递——仅 WARN，不算平台故障）；
- 出站超时：消息发送连接 5s、请求 **30s**（主动消息响应实测可 >10s，过短会误报）；控制面（换 token / 网关地址）连接 5s、请求 10s（`QqApiClient`）；
