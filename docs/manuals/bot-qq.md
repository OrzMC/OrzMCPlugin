...
- **被动回复（带 `msg_id`）**：官方（2026-09）有效窗口 **群聊 5 分钟 / 单聊 60 分钟**，每条源消息最多回复 **群聊 5 次 / 单聊 4 次**（超限平台返 `40034128`）；插件会持续递增 `msg_seq`（官方要求，**相同 `msg_id` + `msg_seq` 重复发送会失败** `40054005`），并在超过次数上限时打出含 `40034128` 与上限值的 WARN（便于定位“列表只发出一条/翻页失败”）；
