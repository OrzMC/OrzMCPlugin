...
| R7 | **单条文本上限常量表**：各平台单条上限（TG 4096 / DC 2000 已官方核实；飞书文本 150KB、卡片 30KB；**QQ 官方未公开数字（错误码 40054007），按字节配置化待沙箱实测**）对接现有 formatter 分段 | 各 adapter |
...
| I2 | QQ R7 数字固化（D14） | 被动回复窗口/次数（已固化：群 5min/5 次、单聊 60min/4 次）、主动消息频控（已固化：群 60qpm/1000 条/天）、**单条文本上限仍待实测**（官方未公开数字，现按字节配置化 + 截断）| 常量表落地 + 手册同步（部分已完成）| 🔶 文本上限待实测 |
