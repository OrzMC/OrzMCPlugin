...
| R7 | **单条文本上限常量表**：各平台单条上限（TG 4096 / DC 2000 已官方核实；飞书文本 150KB、卡片 30KB；**QQ 2026-09 实测 ≥150KB（96KB ASCII / 150KB 中文均投递成功），默认 32KB 仅防御性上限**）对接现有 formatter 分段 | 各 adapter |
...
| I2 | QQ R7 数字固化（D14） | 被动回复窗口/次数（群 5min/5 次、单聊 60min/4 次）、主动消息频控（群 60qpm/1000 条/天）、**单条文本上限（2026-09 实测 ≥150KB；默认 32KB 防御性上限）** —— 全部已固化 | 常量表落地 + 手册同步 | ✅ 已完成（2026-09） |
