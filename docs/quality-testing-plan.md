...
| **guide** | GuideService / GuideBookConfigParser / GuideLineMarkup / GuideBookRenderer / GuideBookMigrator | `/guide` 新手书；首次进入自动发放；v2 简化格式（pages + 行记号，兼容旧 content 格式）；旧格式启动自动迁移（备份 `.bak` + 回读校验 + 幂等）；解析缓存 + `/orzmc config reload` 即改即生效；页数/字符上限降级；startup 健康校验带页号/行号 | 单测+集成+E2E |
...
| guidebook | 6 类 | **91%** | ✅ | 简写记号（转义/不成对降级）、v1↔v2 归一化、颜色口径、页上限降级、旧格式迁移（备份/幂等/回读校验）、文档-模板一致性 |
