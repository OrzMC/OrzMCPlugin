# OrzMC 插件功能清单
> **状态：现行**（功能清单唯一权威；bot 平台操作手册见 `docs/manuals/`，§2 为概览与索引） ｜ **最后更新**：2026-09-16
...
### 9.2 内容配置（v2 简化格式）
- 一行 = 一句话、一页 = 一个 `-` 列表；样式/链接用 3 个记号：`**粗体**`、`<u>下划线</u>`、`[文字](https://地址 '悬停提示')`
- 需要颜色或精细控制时用全写法（`text:` / `link:` + `bold` / `underlined` / `color` / `newline_count`），两种写法可混用
- 旧 `content:` 格式仍然可读；启动时自动迁移为 `pages:` 格式（原文件备份为 `guide_book.yml.bak`，迁移前做一致性回读校验，失败则保留原文件）
- 改完执行 `/orzmc config reload` 即生效（无需重启）；页数（100）/单页字符数（1024）超限时省略末尾内容并告警，不会导致 `/guide` 报错
- 格式问题（记号不成对、链接写法不完整、颜色非法等）在启动健康检查与控制台日志中带**页号/行号**上报，内容按纯文本兜底不丢

> 3 分钟上手手册（速查表 / 全写法 / 旧格式对照 / 排查清单）：[新手指南书配置手册](guide-book.md)
...
|------|------|------|------|
| `/tpbow` | `/tpb` | 获取传送弓 | 通用 |
| `/guide` | — | 打开新手指南书（内容改动经 `/orzmc config reload` 即改即生效） | 通用 |
...
- **guide_book.yml** — 新手指南书内容配置（v2 简化格式：`pages` 分页 + 行记号；旧 `content` 格式启动时自动迁移并备份 `.bak`，见 [手册](guide-book.md)）
