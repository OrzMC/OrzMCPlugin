...
class CommandOutputAssemblerTest {
    ...
    private static final String TRUNC_TEMPLATE = "…（输出过长，已截断，共 {count} 行）";

    @Test
    void assemble_syncLinesOnly_joinsWithNewline() {
        String result = CommandOutputAssembler.assemble(List.of("a", "b"), List.of(), 30, TRUNC_TEMPLATE);
        assertEquals("a\nb", result);
    }

    @Test
    void assemble_logLinesAppendedAfterSyncLines() {
        String result = CommandOutputAssembler.assemble(List.of("sync1"), List.of("log1", "log2"), 30, TRUNC_TEMPLATE);
        assertEquals("sync1\nlog1\nlog2", result);
    }
    ...
    void assemble_filtersIssuedServerCommandNoise() {
        String result = CommandOutputAssembler.assemble(
                List.of(), List.of("Rcon issued server command: /list", "real output"), 30, TRUNC_TEMPLATE);
    ...
    void assemble_filtersPlayerChatLines() {
        String result =
                CommandOutputAssembler.assemble(List.of(), List.of("<Steve> 大家好", "real output"), 30, TRUNC_TEMPLATE);
        assertEquals("real output", result);
    }
    ...
    void assemble_keepsCommandOutputWithInlineBracketAngle() {
        ...
        String result =
                CommandOutputAssembler.assemble(List.of("> /luckperms user <user>"), List.of(), 30, TRUNC_TEMPLATE);
        assertEquals("> /luckperms user <user>", result);
    }

    @Test
    void assemble_nonPositiveMaxLines_clampedToOne() {
        String result = CommandOutputAssembler.assemble(List.of("a", "b"), List.of(), 0, TRUNC_TEMPLATE);
        assertEquals("a\n…（输出过长，已截断，共 2 行）", result);
    }

    @Test
    void assemble_filtersBlankLines() {
        String result = CommandOutputAssembler.assemble(List.of("a", " ", ""), List.of(), 30, TRUNC_TEMPLATE);
    ...
    void assemble_deduplicatesSyncAndLogOverlap() {
        String result = CommandOutputAssembler.assemble(
                List.of("same", "unique"), List.of("same", "extra"), 30, TRUNC_TEMPLATE);
        assertEquals("same\nunique\nextra", result);
    }

    @Test
    void assemble_trimsLines() {
        String result = CommandOutputAssembler.assemble(List.of("  padded  "), List.of(), 30, TRUNC_TEMPLATE);
    ...
    void assemble_allNoise_returnsEmpty() {
        String result =
                CommandOutputAssembler.assemble(List.of("issued server command: /x"), List.of(), 30, TRUNC_TEMPLATE);
        assertEquals("", result);
    }

    @Test
    void assemble_nullInputs_returnsEmpty() {
        assertEquals("", CommandOutputAssembler.assemble(null, null, 30, TRUNC_TEMPLATE));
    }

    @Test
    void assemble_underMaxLines_returnsAll() {
        String result = CommandOutputAssembler.assemble(List.of("a", "b", "c"), List.of(), 5, TRUNC_TEMPLATE);
        assertEquals("a\nb\nc", result);
    }
    ...
    void assemble_overMaxLines_truncatesWithNotice() {
        ...
        String result = CommandOutputAssembler.assemble(lines, List.of(), 3, TRUNC_TEMPLATE);
